...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="AIOS runtimed managed-worker smoke harness")
...
def main() -> int:
    ...
    if not unix_rpc_supported():
        print("runtimed managed worker smoke skipped: unix rpc transport unsupported on this platform")
        return 0
    runtimed = resolve_binary("runtimed", args.runtimed, args.bin_dir)
    ensure_binary(runtimed, "aios-runtimed")

    temp_root = Path(tempfile.mkdtemp(prefix="aios-runtimed-managed-", dir="/tmp" if Path("/tmp").exists() else None))
    ...
    runtime_profile.write_text(
        textwrap.dedent(
            f"""\
            profile_id: managed-worker-smoke
            scope: system
    ...
    try:
        ...
        require(
            any(note == "managed_worker_count=2" for note in health["notes"]),
        ...
        require(
            any(note == "managed_worker.local-npu=ready" for note in health["notes"]),
        ...
        require(backend_map["local-gpu"]["availability"] == "available", "managed gpu backend availability mismatch")
        require(
            backend_map["local-gpu"]["activation"] == "configured-unix-worker",
            "managed gpu backend activation mismatch",
        )
        require(backend_map["local-npu"]["availability"] == "available", "managed npu backend availability mismatch")
        require(
            backend_map["local-npu"]["activation"] == "configured-unix-worker",
            "managed npu backend activation mismatch",
        )
        ...
        gpu_response = rpc_call(
            ...
            "runtime.infer.submit",
            {
                ...
                "task_id": "task-managed-gpu",
                "prompt": "Run managed gpu worker",
                "model": "smoke-model",
                "preferred_backend": "local-gpu",
            },
            timeout=args.timeout,
        )
        require(gpu_response["backend_id"] == "local-gpu", "managed gpu response backend mismatch")
        require(gpu_response["route_state"] == "local-gpu-worker-v1", "managed gpu route mismatch")
        ...
        npu_response = rpc_call(
            ...
            {
                "session_id": "session-managed",
                "task_id": "task-managed-npu",
                "prompt": "Run managed npu worker",
                "model": "smoke-model",
                "preferred_backend": "local-npu",
            },
            timeout=args.timeout,
        )
        require(npu_response["backend_id"] == "local-npu", "managed npu response backend mismatch")
        require(npu_response["route_state"] == "local-npu-worker-v1", "managed npu route mismatch")
    except Exception:
        failed = True
        raise
    finally:
        ...
        if args.keep_state:
            print(f"kept managed worker smoke state at: {temp_root}")
        else:
            shutil.rmtree(temp_root, ignore_errors=True)

    print("runtimed managed worker smoke passed")
    return 0
