...
STAGING_DIR="${AIOS_RECOVERY_IMAGE_STAGING_DIR:-$ROOT_DIR/.cache/aios-recovery-image-src}"
OUTPUT_DIR="${AIOS_RECOVERY_IMAGE_OUTPUT_DIR:-$BASE_IMAGE_DIR/recovery.output}"
CMDLINE_FILE="${AIOS_RECOVERY_KERNEL_CMDLINE_FILE:-$BASE_IMAGE_DIR/recovery/kernel-command-line.txt}"
...
RECOVERY_STEM="${AIOS_RECOVERY_IMAGE_OUTPUT_STEM:-aios-qemu-x86_64-recovery}"

recovery_artifacts_ready() {
  [[ -f "$OUTPUT_DIR/$RECOVERY_STEM.raw" ]] || return 1
  [[ -f "$OUTPUT_DIR/$RECOVERY_STEM.efi" ]] || return 1
  [[ -f "$OUTPUT_DIR/$RECOVERY_STEM.initrd" ]] || return 1
  [[ -f "$OUTPUT_DIR/$RECOVERY_STEM.vmlinuz" ]] || return 1
}
...
manifest = {
    'profile': 'qemu-x86_64-recovery',
    'output_dir': str(output_dir),
    'images': images,
    'default_target': 'aios-recovery.target',
    'kernel_command_line_asset': 'aios/image/recovery/kernel-command-line.txt',
}
(output_dir / 'recovery-image-manifest.json').write_text(json.dumps(manifest, indent=2) + '\n')
...
derive_from_base_if_possible() {
  ...
  if recovery_artifacts_ready; then
    printf 'reusing existing recovery image artifacts in %s\n' "$OUTPUT_DIR" >&2
    write_manifest
    exit 0
  fi
  ...
  printf 'deriving recovery image artifacts from base output %s\n' "$BASE_OUTPUT_DIR" >&2
  cp -f "$BASE_OUTPUT_DIR/$BASE_STEM.raw" "$OUTPUT_DIR/$RECOVERY_STEM.raw"
  cp -f "$BASE_OUTPUT_DIR/$BASE_STEM.efi" "$OUTPUT_DIR/$RECOVERY_STEM.efi"
  cp -f "$BASE_OUTPUT_DIR/$BASE_STEM.initrd" "$OUTPUT_DIR/$RECOVERY_STEM.initrd"
  cp -f "$BASE_OUTPUT_DIR/$BASE_STEM.vmlinuz" "$OUTPUT_DIR/$RECOVERY_STEM.vmlinuz"
...
for line in text.splitlines():
    ...
    lines.append(line)
if not replaced:
    raise SystemExit('failed to replace base kernel target with recovery cmdline')
