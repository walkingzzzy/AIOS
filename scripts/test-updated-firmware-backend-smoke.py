...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="AIOS updated firmware-backend cross-restart smoke harness"
    )
...
def write_boot_state(
    ...
    current_slot_path.write_text(f"{slot}\n")
...
def main() -> int:
    ...
    temp_root = Path(tempfile.mkdtemp(prefix="aios-updated-firmware-"))
    ...
    current_slot_path = boot_dir / "current-slot"
    boot_success_marker_path = state_root / "boot-success"
    firmware_context_path = state_root / "firmware-context.txt"
    fake_firmwarectl_path = state_root / "fake-firmwarectl.sh"
    ...
    fake_firmwarectl_path.write_text(
        "#!/bin/sh\n"
        "set -eu\n"
        "context=\"$AIOS_UPDATED_TEST_FIRMWARECTL_CONTEXT\"\n"
        "state_dir=\"${AIOS_UPDATED_TEST_FIRMWARE_STATE_DIR:-}\"\n"
        "verb=\"${1:-}\"\n"
        "if [ $# -gt 0 ]; then\n"
        "  shift\n"
        "fi\n"
        ...
        "      current=$(head -n 1 \"$state_dir/current-slot\")\n"
        "    fi\n"
        "    printf 'current_slot=%s\\n' \"$current\"\n"
        "    ;;\n"
        "  set-active)\n"
        "    printf '%s' \"$verb|$slot\" > \"$context\"\n"
    ...
    fake_firmwarectl_path.chmod(0o755)
    ...
    env.update(
        {
            ...
            "AIOS_UPDATED_BOOT_BACKEND": "firmware",
            "AIOS_UPDATED_FIRMWARECTL_BIN": str(fake_firmwarectl_path),
            ...
            "AIOS_UPDATED_TEST_FIRMWARECTL_CONTEXT": str(firmware_context_path),
            "AIOS_UPDATED_TEST_FIRMWARE_STATE_DIR": str(boot_dir),
        }
    )
    ...
    try:
        ...
        require(
            firmware_context_path.read_text() == "set-active|b",
            "firmwarectl apply context mismatch",
        )
        ...
        require((boot_dir / "next-slot").read_text().strip() == "b", "next-slot should be b")
        terminate(process)
        print_log("updated firmware stage", process)
        process = None
        ...
        require(
            firmware_context_path.read_text() == "mark-good|b",
            "firmwarectl mark-good context mismatch after booting slot b",
        )
        ...
        require(
            restarted_boot.get("staged_slot") is None,
            "startup sync should clear staged slot after successful boot",
        )
        require(restarted_boot["boot_success"] is True, "startup sync should persist boot success")
        require(restarted_deployment["status"] == "up-to-date", "deployment should converge")
        require(
            restarted_deployment["current_version"] == "0.1.1",
            "deployment should promote target version",
        )
        require(restarted_deployment.get("pending_action") is None, "pending action should be cleared")
        require(restarted_surface.get("current_slot") == "b", "recovery surface current slot mismatch")
        require(restarted_surface.get("last_good_slot") == "b", "recovery surface last good mismatch")
        ...
        require(
            firmware_context_path.read_text() == "set-active|a",
            "firmwarectl rollback context mismatch",
        )
        ...
        require(rollback_boot.get("staged_slot") == "a", "rollback should stage slot a")
        terminate(process)
        print_log("updated firmware rollback", process)
        process = None
        ...
        wait_for_health(socket_path, args.timeout)
        require(
            firmware_context_path.read_text() == "mark-good|a",
            "firmwarectl mark-good context mismatch after rollback boot",
        )
        ...
        final_recovery_record = read_json(
            Path(state_root / "recovery" / f"{apply_result['recovery_ref']}.json")
        )
        ...
        require(final_boot.get("staged_slot") is None, "final staged slot should be cleared")
        require(final_deployment["status"] == "up-to-date", "final deployment should converge")
        require(
            final_deployment["current_version"] == "0.1.0",
            "final deployment should restore original version",
        )
        require(final_deployment.get("pending_action") is None, "final pending action should clear")
        require(final_recovery_record["status"] == "rolled-back", "recovery point should be rolled back")
        require(final_surface.get("current_slot") == "a", "final surface current slot mismatch")
        require(final_surface.get("last_good_slot") == "a", "final surface last good mismatch")

        print(
            json.dumps(
                {
                    "status": "passed",
                    "boot_backend": "firmware",
                    "apply_recovery_id": apply_result["recovery_ref"],
                    "final_slot": final_boot["current_slot"],
                    "final_version": final_deployment["current_version"],
        ...
        failed = True
        print(f"updated firmware backend smoke failed: {error}")
        return 1
    finally:
        terminate(process)
        print_log("updated firmware final", process)
