...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Jetson platform reference accelerator worker")
...
def build_response(backend: str, request: dict) -> dict:
    ...
    content = "" if rejected else f"jetson reference {backend} worker completed {request.get('task_id', 'task')}"
    reason = "worker rejected prompt directive" if rejected else "jetson reference accelerator worker executed request"
    return {
