...
- 已补：compositor 现已把 shell role / interaction mode / window policy 正式收敛为结构化 contract，session summary 会额外暴露 `workspace_toplevel_mode`、`modal_surface_count`、`blocked_surface_count`、`shell_role_counts`、`interaction_mode_counts`、`window_policy_counts`、`topmost_slot_id` 以及 `drm_connector_name` / `drm_output_width` / `drm_output_height` / `drm_refresh_millihz`；同时 `xdg_toplevel` configure 已按 `workspace-maximized|fullscreen`、`modal-dialog`、`floating-dialog`、`floating-utility` 这些策略输出正式状态，`release-shell-profile` 与 `release-compositor.conf` 也已支持 `drm_preferred_connector` / 输出宽高 / 刷新率偏好
- 已补：窗口管理层现在已正式接入 compositor 主链，支持 `workspace_count` / `active_workspace_id` / `output_layout_mode` / `window_state_path` 这些配置与 summary 契约；nested 路径支持基于 `virtual_outputs` 的逻辑多输出编排，launcher 滚轮切 workspace、task-surface 横向滚轮切 active output、floating/dialog/workspace 窗口的 compositor-managed drag/resize，以及基于 `window_state_path` 的持久窗口布局/工作区恢复
- 已补：窗口管理交互继续推进，floating/workspace 窗口现在支持逻辑跨输出 drag-drop 自动迁移 output 归属、middle-click 最小化与 task-surface 背景 middle-click 恢复最近最小化窗口，以及 right-click 将窗口送到下一个 workspace；session summary 也已补 `minimized_window_count` / `window_minimize_count` / `window_restore_count` / `last_minimized_window_key` / `last_restored_window_key` / `workspace_window_counts`
- 已补：`shell_session.py` 现已把 `standalone` / `compositor` / `standalone-fallback` 收敛到统一 host 选择与环境注入链路，正式暴露 `host_command` / `gtk_host_command` / `tk_host_command` / `host_launch_mode` / `panel_clients_enabled` 等 session plan contract；GTK/Tk 外部 host、panel clients 与 fallback 不再各走一套入口。
- 已补：`launcher` / `task-surface` / `notification-center` 现在都会消费 compositor runtime/window state，正式暴露 `active-workspace-windows`、`minimized-windows`、`renderable_output_count`、`non_renderable_output_count`、`release_grade_output_status` 等工作区/窗口生命周期视图；`shellctl` / panel / acceptance smoke 已覆盖 focus / restore / workspace 摘要链路。
- 已补：`scripts/test-shell-session-smoke.py`、`scripts/test-shell-acceptance-smoke.py`、`scripts/test-shell-stability-smoke.py`、`scripts/test-shell-compositor-acceptance-smoke.py` 现在默认把 `AIOS_SHELL_SESSION_TEMP_ROOT` 指向仓库 `.tmp/`，受限 Windows/CI 环境也能稳定收集 session/compositor evidence。
- 未完全收口：真实多头 DRM/KMS scanout 下的每输出独立 pageflip / render loop、跨输出 drag-drop 的真多头 scanout 仍需 Linux 真机/libseat/DRM 环境补最终证据；当前 formal panels 已补窗口列表、最小化恢复与输出状态视图，但最小化托盘等独立 shell surface 仍属于桌面深化项。

当前判断：`P0/P1/P2 开发与验收资产已收口，真实多头真机证据待补`
...
- `shellctl.py`：统一 profile 解析、跨组件状态汇总、formal panel 与透传调用入口
...
1. 在 Linux 真机上补 `drm-kms` 真多头 pageflip/render loop 与跨输出 drag-drop 的 release-grade 证据
2. 把当前 launcher/task-surface 的窗口列表与最小化恢复视图继续沉淀为更独立的 tray / shell surface
3. 继续补 operator-facing 运行说明、release profile 部署指引与真实图形后端约束
...
- shell release profile smoke：`python3 scripts/test-shell-release-profile-smoke.py`
- live/provider smoke：`python3 scripts/test-shell-live-smoke.py` / `python3 scripts/test-shell-provider-smoke.py` / `python3 scripts/test-shell-control-clients-live-smoke.py` / `python3 scripts/test-shell-session-policy-registry-flow-smoke.py`；这些 harness 依赖 `updated` / `deviced` / `sessiond` / `policyd` / `agentd` 的 Unix RPC 服务二进制，非 Unix/Windows 开发机默认输出 `skip` 并返回 0
