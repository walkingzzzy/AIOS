...
- 未有：真实多头 DRM/KMS scanout 下的每输出独立 pageflip / render loop、跨输出 drag-drop 的真多头 scanout 闭环、以及更完整的工作区生命周期（最小化托盘、窗口列表、最小化/恢复的可视化 shell surface）；当前已具备逻辑多输出编排与持久窗口管理，但还不是完整桌面环境
