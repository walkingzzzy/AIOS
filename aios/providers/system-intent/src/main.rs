...
async fn probe_sessiond_health(state: &AppState, attempts: usize) -> (String, Option<String>) {
    for attempt in 0..attempts {
        match crate::clients::fetch_sessiond_health(state) {
            Ok(_) => return ("available".to_string(), None),
            Err(error) if attempt + 1 < attempts => {
                tokio::time::sleep(Duration::from_millis(100)).await;
                if !state.config.sessiond_socket.exists() {
                    continue;
                }
                tracing::debug!(?error, "sessiond probe failed during startup, retrying");
            }
    ...
    (
        ...
        Some("sessiond probe exhausted".to_string()),
    )
}
...
async fn sync_registry_state_once(state: &AppState, registration_succeeded: &mut bool) {
    ...
    let (status, last_error) = probe_sessiond_health(state, 1).await;
    match crate::clients::report_provider_health(state, &status, last_error.clone()) {
        Ok(_) => {
            state
                .registry_sync
...
async fn main() -> anyhow::Result<()> {
    aios_core::logging::init("system-intent-provider");
    ...
    tracing::info!(
        ...
        "starting aios system-intent provider"
    );
    emit_trace(
        &state,
        ...
        json!({
            ...
            "sessiond_socket": config.sessiond_socket.display().to_string(),
        }),
        vec!["lifecycle=startup".to_string()],
    );

    let registration_succeeded = self_register_with_registry(&state, 10).await;
    let (initial_status, initial_error) = probe_sessiond_health(&state, 10).await;
