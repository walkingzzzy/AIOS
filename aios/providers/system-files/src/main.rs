...
pub struct AppState {
    ...
    pub audit_writer: audit::AuditWriter,
...
impl AppState {
    fn health(&self) -> HealthResponse {
        let mut notes = vec![
            ...
            format!("audit_log_path={}", self.config.audit_log_path.display()),
            format!(
                "observability_log_path={}",
                self.config.observability_log_path.display()
            ),
            format!("max_preview_bytes={}", self.config.max_preview_bytes),
            format!(
            ...
            format!(
                "max_delete_affected_paths={}",
                self.config.max_delete_affected_paths
            ),
        ];
...
fn reserve_startup_capacity_for_tests(state: &AppState) {
    if state.config.test_startup_reserve_ms == 0 {
        return;
    }

    let permit = match state.concurrency_budget.try_acquire("startup-reserve") {
        Ok(permit) => permit,
        Err(error) => {
            tracing::warn!(
                ?error,
                "failed to reserve startup concurrency slot for tests"
            );
            return;
        }
    };

    let hold_ms = state.config.test_startup_reserve_ms;
...
async fn sync_registry_state_once(state: &AppState, registration_succeeded: &mut bool) {
    ...
    match crate::clients::report_provider_health(state, "available", None) {
        Ok(_) => {
            state.registry_sync.record_health_report("available", None);
            emit_health_event(
                state,
                "background",
                "available",
                "reported provider health to agentd",
                vec![
                    "registry_status=available".to_string(),
...
async fn main() -> anyhow::Result<()> {
    aios_core::logging::init("system-files-provider");

    let config = config::Config::load().await?;
    let state = AppState {
        config: config.clone(),
        started_at: Utc::now(),
        audit_writer: audit::AuditWriter::with_provider_id(
    ...
    tracing::info!(
        ...
        "starting aios system-files provider"
    );
    emit_trace(
        &state,
    ...
    reserve_startup_capacity_for_tests(&state);
    report_registry_health(&state, "available", None, "startup", 10).await;
