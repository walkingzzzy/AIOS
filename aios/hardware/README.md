...
- bring-up kit 现已包含：canonical hardware profile 副本、Tier 1 profile、formal nominated machine profile 副本、support matrix、known limitations、实机执行 README、install/rollback checklist、hardware validation report template、device/multimodal 验收区段、evidence index template、pull/evaluate/render helper scripts，以及一键 `collect-and-render-hardware-validation.sh` wrapper；`collect-aios-device-validation.py` 现可把 `deviced` / `device-metadata` 快照、`backend-state.json`、renderer args 与 vendor runtime evidence 一并整理为报告渲染输入
...
3. 优先使用 bring-up kit 里的 `collect-and-render-hardware-validation.sh`，或先运行 `collect-device-validation.sh` 收集 `deviced` / `device-metadata` / backend evidence，再分别调用 evaluator / renderer 生成 bring-up 报告与 evidence index
